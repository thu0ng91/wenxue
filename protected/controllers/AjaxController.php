...
class AjaxController extends Q {
    ...
    public function actionAddComment() {
        ...
        if ($this->uid) {
            ...
            $uid = $this->uid;
            if(!$this->userInfo['isAdmin']){//不是管理员                
                if (zmf::actionLimit('add-comments', $type.'-'.$keyid, 10, 86400, true)) {
                    $this->jsonOutPut(0, '一天内只允许 10 条评论');
                }
            }            
        } else {
            if (!$username) {
            ...
            $uid = 0;
            if (zmf::actionLimit('add-comments', $type.'-'.$keyid, 5, 86400, true)) {
                $this->jsonOutPut(0, '未登录时本篇文章每天只允许 5 条评论');
            }
        }
